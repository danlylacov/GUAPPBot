...
async def directions_menu(message: types, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, "Вы находитесь в меню обзора направлений!", reply_markup=types.ReplyKeyboardRemove())
...
async def show_focus(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if (int(callback_query.data), ) in focus:
        ...
        await bot.send_message(callback_query.from_user.id, message, parse_mode="HTML", reply_markup=back_inline_keyboard)
